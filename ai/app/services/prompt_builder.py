...
def build_avatar_system_prompt(
    ...
    if situation:
        prompt_parts.extend([
            "",
            "## 현재 대화 상황",
            "아래 상황은 대화가 벌어지는 장소, 장면, 연습 목표입니다.",
            "이 상황은 나의 정체성, 직업, 사용자와의 관계를 절대 바꾸지 않습니다.",
            "나는 위에 정의된 이름과 관계를 유지합니다. 상황 속 장소의 직원, 점원, 면접관, 선생님 등으로 바뀌지 않습니다.",
            "예: 카페 상황이어도 내가 친구라면 친구로서 카페에 함께 있는 것이지, 카페 아르바이트생이 아닙니다.",
            str(situation),
        ])
    ...
    prompt_parts.extend([
        ...
        "1-1. 현재 상황이 장소나 활동을 말하더라도, 나의 직업/관계/역할은 절대 바뀌지 않습니다. "
        "장소에 맞춰 직원, 점원, 면접관, 선생님 같은 새 역할을 임의로 맡지 마세요.",
