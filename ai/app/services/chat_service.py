...
- has_errors가 false이면 corrected_message는 null, corrections는 []로 두세요. 단, 학습 가치가 있는 다른 자연스러운 표현은 natural_alternatives에 1~2개 넣어도 됩니다.
...
- natural_alternatives는 선택사항입니다. 정말 더 자연스럽고 학습 가치가 있을 때 1~3개 제안하세요.
- 오류가 2개 이상이거나 문장 전체를 크게 다듬었다면 서로 다른 패턴의 개선 표현을 2~3개 제안하세요.
- 오류가 없고 현재 문장이 이미 자연스러워도, 사용자가 문장 구성이나 표현을 더 배울 수 있다면 같은 의미의 대안 표현을 1~2개 제안해도 됩니다.
...
- 오류 없으면 corrections 빈 배열, has_errors false. natural_alternatives는 학습용 표현이 있을 때만 포함하세요."""
...
def _is_valid_natural_alternative(
    ...
    if not expression:
        return False
    if is_suspicious_semantic_expansion(user_message, expression):
        return False
    if len(expression) < 2:
        return False
...
_POLITE_ENDINGS   = ["어요", "아요", "이에요", "예요", "해요", "세요",
                     "네요", "군요", "죠", "나요", "가요", "래요",
                     "데요", "을게요", "ㄹ게요", "겠어요"]
_INFORMAL_ENDINGS = ["싶음", "싶다", "겠음", "겠다", "없음", "없다",
                     "있음", "있다", "예정임", "함", "한다", "됨", "된다",
                     "이야", "야", "이어", "어", "아", "지", "니",
                     "냐", "거야", "잖아", "이잖아", "구나", "군",
                     "을게", "ㄹ게", "자", "해", "래", "네",
...
_COMMON_TYPO_FIXES = [
    ...
    ("조윯", "조율", "'조윯'보다 '조율'이 맞는 표기입니다."),
    ("시긴", "시간", "'시긴'보다 '시간'이 맞는 표기입니다."),
    ("홗인", "확인", "'홗인'보다 '확인'이 맞는 표기입니다."),
...
_INFORMAL_TO_POLITE = {
    ...
    "을까": "을까요", "ㄹ까": "ㄹ까요", "싶음": "싶어요", "싶다": "싶어요",
    "없음": "없어요", "없다": "없어요", "있음": "있어요", "있다": "있어요",
    "한다": "해요", "된다": "돼요", "함": "해요", "임": "이에요", "음": "어요",
    "야": "요", "해": "해요", "어": "어요", "아": "아요",
    "지": "지요", "군": "군요", "네": "네요", "래": "래요", "자": "시죠",
}

_INFORMAL_TO_FORMAL = {
    "이야": "입니다", "거야": "겁니다", "싶음": "싶습니다", "싶다": "싶습니다",
    "없음": "없습니다", "없다": "없습니다", "있음": "있습니다", "있다": "있습니다",
    "한다": "합니다", "된다": "됩니다", "함": "합니다", "임": "입니다", "음": "습니다", "야": "습니다",
    "해": "합니다", "어": "습니다", "아": "습니다",
    "지": "지요", "군": "군요", "네": "네요",
}
...
def _prefer_rule_correction(
    ...
    if is_generic_greeting_correction(user_message, clova_corrected):
        return rule_corrected
...
def is_generic_greeting_correction(user_message: str, candidate: Optional[str]) -> bool:
    candidate = (candidate or "").strip()
    if candidate not in _GENERIC_GREETING_CORRECTIONS:
        return False
    return not re.search(r"(안녕|반가워|처음 뵙|오랜만)", user_message or "")
...
_KOREAN_PARTICLES = (
    "으로부터", "에게서", "에서", "에게", "께서", "으로", "부터", "까지",
    "보다", "처럼", "만큼", "하고", "이나", "나", "이랑", "랑",
    "은", "는", "이", "가", "을", "를", "에", "께", "로", "와", "과",
    "도", "만",
)

_TIME_NUMERAL_CORRECTIONS = {
    "일": "한",
    "이": "두",
    "삼": "세",
    "사": "네",
...
_UNSAFE_ADDED_MODIFIERS = {
    "매우", "아주", "너무", "정말", "진짜", "완전", "굉장히", "엄청", "상당히",
}

_CONNECTIVE_TOKENS = {
    "하지만", "그런데", "그러나", "그리고", "그래서", "그러면", "따라서",
}

_RELATION_MARKER_TOKENS = {
    "대해", "대한", "관해", "관한",
}

_SUBMISSION_OBJECTS = "과제|숙제|보고서|자료|신청서|확인서|서류"
_LISTENER_TITLE_PATTERN = r"[가-힣]{1,12}님(?:께서|께|에게|한테)"


def _word_counts(text: str) -> Counter:
    return Counter(re.findall(r"[가-힣A-Za-z0-9]+", text or ""))


def _listener_title_base_counts(text: str) -> Counter:
    counts: Counter = Counter()
    for match in re.finditer(r"([가-힣]{1,12}님)(?:께서|께|에게|한테)?", text or ""):
...
def is_suspicious_semantic_expansion(source_text: str, candidate_text: str) -> bool:
    ...
    source_counts = _word_counts(source_text)
    candidate_counts = _word_counts(candidate_text)
    for token in _UNSAFE_ADDED_MODIFIERS:
        if candidate_counts[token] > source_counts[token]:
            return True

    source_title_counts = _listener_title_base_counts(source_text)
    candidate_title_counts = _listener_title_base_counts(candidate_text)
    for title, candidate_count in candidate_title_counts.items():
        if candidate_count > 1 and candidate_count > max(1, source_title_counts.get(title, 0)):
            return True

    if re.search(r"(합니다|됩니다|습니다)[.!?…。！？]?\s*(합니다|됩니다|습니다)", candidate_text):
        return True
    if (
        re.search(r"해야\s*(?:돼요|해요)\s+하지만", source_text)
        and re.search(r"해야\s*(?:합니다|됩니다)[.!?…。！？]?\s+하지만", candidate_text)
    ):
        return True
    if (
        re.search(r"해야\s*(?:돼요|해요)\s+하지만", source_text)
        and re.search(r"해야\s*(?:합니다|됩니다)만", candidate_text)
    ):
        return True
    if re.search(r"는지[?？]?$", candidate_text.strip()):
        return True

    tokens = re.findall(r"[가-힣A-Za-z0-9]+", candidate_text or "")
    for index, token in enumerate(tokens):
        if index >= 1 and token == tokens[index - 1] and token in _UNSAFE_ADDED_MODIFIERS:
            return True
        if (
            index >= 1
            and token == tokens[index - 1]
            and len(token) >= 2
            and candidate_counts[token] > source_counts[token]
        ):
            return True
...
def is_suspicious_inline_correction(original: str, corrected: str) -> bool:
    original = (original or "").strip()
    corrected = (corrected or "").strip()
    if is_suspicious_semantic_expansion(original, corrected):
        return True
    original_norm = re.sub(r"[.!?…。！？\s]+$", "", original)
    corrected_norm = re.sub(r"[.!?…。！？\s]+$", "", corrected)
    if (
        len(original_norm) >= 4
        and corrected_norm in {"합니다", "습니다", "됩니다", "해요", "돼요", "요"}
        and original_norm.endswith(corrected_norm)
    ):
        return True
    if (
        len(original_norm) >= 4
        and original_norm.endswith(("합니다", "습니다", "됩니다"))
        and corrected_norm in {"합니다", "습니다", "됩니다"}
        and original_norm != corrected_norm
    ):
        return True
    if (
        1 <= len(original_norm) <= 3
        and corrected_norm.endswith(original_norm)
        and corrected_norm != original_norm
        and re.search(
            r"(오전|오후|\d|한|두|세|네|다섯|여섯|일곱|여덟|아홉|열)",
            corrected_norm[: -len(original_norm)],
        )
    ):
        return True
    if original_norm in {"시", "시에"} and corrected_norm != original_norm:
        return True
    if (
        original_norm.endswith(("합니다", "습니다", "됩니다"))
        and corrected_norm.startswith(("합니다", "습니다", "됩니다"))
        and any(token in corrected_norm for token in _CONNECTIVE_TOKENS)
    ):
        return True
    if original_norm.endswith("나요") and corrected_norm.endswith("는지"):
        return True
    if re.match(r"^(제가|저는|저를|저에게|저한테)(?:\s|$)", original_norm) and re.match(_LISTENER_TITLE_PATTERN, corrected_norm):
        return True
    if re.search(r"(내도|써도|가도|먹어도|봐도)\s*돼요", original_norm) and corrected_norm.startswith("해도 "):
        return True
    if original_norm and corrected_norm.endswith(original_norm) and corrected_norm != original_norm:
        added_prefix = corrected_norm[: -len(original_norm)]
        if re.search(r"(합니다|됩니다|습니다|해요|돼요|하지만|그리고|그런데|대해|대한|관해|관한)", added_prefix):
            return True
    if original_norm and original_norm in corrected_norm and corrected_norm != original_norm:
        added_prefix = corrected_norm.split(original_norm, 1)[0]
        if any(token in added_prefix for token in _RELATION_MARKER_TOKENS):
            return True
    if original in _CONNECTIVE_TOKENS and original in corrected and original != corrected:
        if re.search(r"(합니다|됩니다|습니다)[.!?…。！？]?\s*" + re.escape(original), corrected):
            return True
    return False


def choose_safe_corrected_message(
    user_message: str,
    proposed: Optional[str],
    fallback: Optional[str] = None,
) -> Optional[str]:
    proposed = (proposed or "").strip() or None
    fallback = (fallback or "").strip() or None
    if proposed and is_generic_greeting_correction(user_message, proposed):
        proposed = None
    if fallback and is_generic_greeting_correction(user_message, fallback):
        fallback = None
    if proposed and not is_suspicious_semantic_expansion(user_message, proposed):
        return proposed
    if fallback and not is_suspicious_semantic_expansion(user_message, fallback):
        return fallback
    return None
...
def _without_particle(text: str) -> str:
...
def is_particle_adjustment(original: str, corrected: str) -> bool:
    original_norm = _strip_terminal_punctuation(original)
    corrected_norm = _strip_terminal_punctuation(corrected)
    if not original_norm or not corrected_norm or original_norm == corrected_norm:
        return False
    if corrected_norm.startswith(original_norm):
        suffix = corrected_norm[len(original_norm):]
        if suffix in _KOREAN_PARTICLES:
            return True
    return _without_particle(original_norm) == _without_particle(corrected_norm)


def apply_inline_correction_once(text: str, original: str, corrected: str) -> str:
    """Apply one surface correction without creating duplicates like 시간을을."""
    if not text or not original or not corrected or original == corrected:
        return text

    search_from = 0
    while True:
        index = text.find(original, search_from)
        if index < 0:
            return text
        if len(corrected) >= len(original) and text.startswith(corrected, index):
            search_from = index + len(original)
            continue
        if corrected.endswith(original):
            added_prefix = corrected[: -len(original)]
            prefix_start = index - len(added_prefix)
            if added_prefix and prefix_start >= 0 and text[prefix_start:index] == added_prefix:
                search_from = index + len(original)
                continue
        return f"{text[:index]}{corrected}{text[index + len(original):]}"
...
def build_contextual_base_corrections(
    ...
    if re.search(r"면담\s+시간\s+(잡고|정하고|조율하고|바꾸고|옮기고)\s+싶", text):
        corrections.append(InlineCorrection(
            original="시간",
            corrected="시간을",
            type=CorrectionType.GRAMMAR,
            severity=CorrectionSeverity.WARNING,
            explanation="동사 '잡다/정하다/조율하다'의 대상이므로 '시간을'처럼 목적격 조사를 붙이면 더 자연스럽습니다.",
            tip="동작의 대상에는 보통 목적격 조사 '을/를'을 붙입니다.",
        ))

    for sino, native in _TIME_NUMERAL_CORRECTIONS.items():
        for match in re.finditer(rf"(?<![가-힣]){sino}\s*시([에가은는를]?)(?![가-힣])", text):
            particle = match.group(1) or ""
            corrections.append(InlineCorrection(
                original=match.group(0),
                corrected=f"{native} 시{particle}",
                type=CorrectionType.VOCABULARY,
                severity=CorrectionSeverity.WARNING,
                explanation="시간을 말할 때는 '삼 시'보다 '세 시'처럼 고유어 수사를 쓰는 것이 자연스럽습니다.",
            ))

    for match in re.finditer(r"([가-힣]+)(을|를|이|가|은|는|에)\2", text):
        corrections.append(InlineCorrection(
            original=match.group(0),
            corrected=f"{match.group(1)}{match.group(2)}",
            type=CorrectionType.GRAMMAR,
            severity=CorrectionSeverity.WARNING,
            explanation="같은 조사가 반복되면 어색하므로 한 번만 쓰는 것이 자연스럽습니다.",
        ))

    for match in re.finditer(r"잡도", text):
        corrections.append(InlineCorrection(
            original=match.group(0),
            corrected="잡아도",
            type=CorrectionType.GRAMMAR,
            severity=CorrectionSeverity.WARNING,
            explanation="'잡도'가 아니라 동사 어간에 맞게 '잡아도'로 활용해야 자연스럽습니다.",
        ))

    for match in re.finditer(r"방향성\s+상담하고", text):
        corrections.append(InlineCorrection(
            original=match.group(0),
            corrected="방향성에 대해 상담하고",
            type=CorrectionType.GRAMMAR,
            severity=CorrectionSeverity.WARNING,
            explanation="'방향성 상담하고'보다 '방향성에 대해 상담하고'처럼 대상 관계를 밝혀 주면 더 자연스럽습니다.",
            tip="'상담하다' 앞에는 보통 '-에 대해'나 목적격 조사를 함께 씁니다.",
        ))

    for match in re.finditer(r"[가-힣]{1,12}님으로(?=\s)", text):
        corrections.append(InlineCorrection(
            original=match.group(0),
            corrected=f"{match.group(0)[:-2]},",
            type=CorrectionType.EXPRESSION,
            severity=CorrectionSeverity.WARNING,
            explanation="상대를 부르는 호칭 뒤에는 '-으로'보다 쉼표로 호명하는 편이 자연스럽습니다.",
        ))
    ...
    if "면담확인서" in text:
        corrections.append(InlineCorrection(
            ...
            type=CorrectionType.SPELLING,
            severity=CorrectionSeverity.WARNING,
            explanation="'면담 확인서'처럼 띄어 쓰면 의미 단위가 더 분명합니다.",
        ))

    submission_match = re.search(rf"({_SUBMISSION_OBJECTS})\s+(?:늦게\s+)?(?:내|제출|보내)", text)
    if submission_match and not re.search(rf"{re.escape(submission_match.group(1))}[을를]", text):
        corrections.append(InlineCorrection(
            original=submission_match.group(1),
            corrected=f"{submission_match.group(1)}를",
            type=CorrectionType.GRAMMAR,
            severity=CorrectionSeverity.WARNING,
            explanation="제출하는 대상에는 '을/를' 같은 목적격 조사를 붙이면 문장 구조가 더 자연스럽습니다.",
        ))

    if re.search(rf"({_SUBMISSION_OBJECTS}).{{0,12}}늦게\s+내도", text):
        corrections.append(InlineCorrection(
            original="내도",
            corrected="제출해도",
            type=CorrectionType.VOCABULARY,
            severity=CorrectionSeverity.WARNING,
            explanation="공식적인 학교 상황에서는 '내다'보다 '제출하다'가 더 정확하고 자연스럽습니다.",
        ))

    if expected_norm == "formal":
        covered_spans = []
        for match in re.finditer(r"해야\s*돼요\s+하지만", text):
            corrections.append(InlineCorrection(
                original=match.group(0),
                corrected="해야 하지만",
                type=CorrectionType.GRAMMAR,
                severity=CorrectionSeverity.WARNING,
                explanation="'해야 돼요 하지만'처럼 문장을 끊기보다 '-해야 하지만'으로 연결하면 더 자연스럽습니다.",
                tip="'하지만' 앞에서는 문장 끝맺음보다 연결 어미를 쓰면 문장이 매끄럽습니다.",
            ))
            covered_spans.append(match.span())

        for match in re.finditer(r"해야\s*(?:돼요|해요)", text):
            ...
            corrections.append(InlineCorrection(
                ...
                type=CorrectionType.SPEECH_LEVEL,
                severity=CorrectionSeverity.WARNING,
                explanation="합쇼체 상황에서는 '해야 돼요'보다 '해야 합니다'로 끝맺는 것이 자연스럽습니다.",
                tip="공식적인 문장에서는 '-해야 합니다'가 더 격식 있게 들립니다.",
            ))

        for match in re.finditer(r"되\?", text):
            corrections.append(InlineCorrection(
                original=match.group(0),
                corrected="되겠습니까?",
                type=CorrectionType.SPEECH_LEVEL,
                severity=CorrectionSeverity.WARNING,
                explanation="합쇼체 상황에서는 허락이나 가능 여부를 물을 때 '되겠습니까?'처럼 정중하게 끝맺는 것이 자연스럽습니다.",
            ))

        for match in re.finditer(r"되나요[?？]", text):
            corrections.append(InlineCorrection(
                original=match.group(0),
                corrected="되겠습니까?",
                type=CorrectionType.SPEECH_LEVEL,
        ...
        for match in re.finditer(r"돼요[?？]", text):
            if any(start <= match.start() and match.end() <= end for start, end in covered_spans):
                continue
            corrections.append(InlineCorrection(
                original=match.group(0),
        ...
        for match in re.finditer(r"돼요(?![?？])", text):
            ...
            corrections.append(InlineCorrection(
                ...
                type=CorrectionType.SPEECH_LEVEL,
                severity=CorrectionSeverity.WARNING,
                explanation="합쇼체 상황에서는 '돼요'보다 '됩니다'로 끝맺으면 더 격식 있게 들립니다.",
            ))

    weather_topic_match = re.search(r"\b(오늘|내일|요즘|이번 주)\s+날씨는\b", text)
    if weather_topic_match:
        time_word = weather_topic_match.group(1)
        corrections.append(InlineCorrection(
            original=weather_topic_match.group(0),
            corrected=f"{time_word}은 날씨가",
            type=CorrectionType.GRAMMAR,
            severity=CorrectionSeverity.WARNING,
            explanation="'오늘 날씨는'보다 '오늘은 날씨가'처럼 시간은 배경으로 두고 날씨를 주어로 세우면 더 자연스럽습니다.",
            tip="'은/는'은 배경이나 화제를, '이/가'는 상태를 설명하는 주어를 표시할 때 자주 씁니다.",
        ))
...
def infer_surface_correction_type(original: str, corrected: str) -> CorrectionType:
    ...
    if is_particle_adjustment(original, corrected):
    ...
    if original_norm.endswith("나요") and corrected_norm.endswith("는지"):
        return CorrectionType.EXPRESSION
    plain_style_endings = ("싶다", "겠다", "없다", "있다", "한다", "된다", "이다")
    polite_formal_endings = ("습니다", "습니까", "어요", "아요", "세요", "십시오", "겠습니다")
    if (
        corrected_level in {"formal", "polite"}
        and (
            original_norm.endswith(plain_style_endings)
            or (
                original_norm.endswith("다")
                and not original_norm.endswith(("니다", "습니다"))
                and difflib.SequenceMatcher(
                    a=original_norm[:-1].replace(" ", ""),
                    b=corrected_norm.replace(" ", ""),
                ).ratio() >= 0.35
            )
        )
        and any(corrected_norm.endswith(e) for e in polite_formal_endings)
    ):
    ...
    if (
        ...
        return CorrectionType.SPEECH_LEVEL
    if corrected_level in {"formal", "polite"} and original_norm.endswith(("되", "돼", "됨")):
        return CorrectionType.SPEECH_LEVEL
    if re.search(r"[가-힣]+도$", original_norm) and re.search(r"[가-힣]+[아어]도", corrected_norm):
        return CorrectionType.GRAMMAR
...
def infer_surface_explanation(original: str, corrected: str, correction_type: CorrectionType) -> str:
    ...
    if correction_type == CorrectionType.GRAMMAR:
        if is_particle_adjustment(original, corrected):
            return "명사 뒤에 알맞은 조사를 붙이면 문장 구조가 더 자연스럽습니다."
        return "문장 구조를 조금 다듬으면 더 자연스럽습니다."
...
def _score_alignment_candidate(original_token: str, candidate: str) -> float:
    ...
    if not original_norm or not candidate_norm:
        return 0.0
    if is_suspicious_inline_correction(original_token, candidate):
        return 0.0
...
def _build_pattern_level_corrections(original_text: str, corrected_text: str) -> List[InlineCorrection]:
    ...
    for sino, native in _TIME_NUMERAL_CORRECTIONS.items():
        for match in re.finditer(rf"(?<![가-힣]){sino}\s*시([에가은는를]?)(?![가-힣])", original_text):
            particle = match.group(1) or ""
            corrected = f"{native} 시{particle}"
            if corrected in corrected_text:
                add(
                    match.group(0),
                    corrected,
                    CorrectionType.VOCABULARY,
                    "시간을 말할 때는 '삼 시'보다 '세 시'처럼 고유어 수사를 쓰는 것이 자연스럽습니다.",
                )
...
def merge_surface_corrections(
    ...
    merged = corrections[:]
    for derived in derive_surface_corrections(original_text, corrected_text):
        replaced = False
        for index, existing in enumerate(merged):
            if existing.original != derived.original:
                continue
            existing_is_spacing_only = (
                existing.type == CorrectionType.SPELLING
                and existing.original.replace(" ", "") == existing.corrected.replace(" ", "")
            )
            if existing_is_spacing_only and existing.corrected.replace(" ", "") in derived.corrected.replace(" ", ""):
                replaced = True
                break
            ...
            if derived_strength > existing_strength or (
                ...
                and derived.type in {
                    ...
                    CorrectionType.HONORIFIC,
                    CorrectionType.VOCABULARY,
                    CorrectionType.SPEECH_LEVEL,
                }
            ):
                merged[index] = derived
                replaced = True
                break
    ...
    return prune_suspicious_corrections(merged, corrected_text)


def build_rule_natural_alternatives(
    user_message: str,
    corrected_message: Optional[str],
    expected_norm: str,
    natural_alternatives: List[NaturalAlternative],
) -> List[NaturalAlternative]:
    reference_message = (corrected_message or user_message or "").strip()
    if not reference_message:
        return natural_alternatives

    output = natural_alternatives[:]
    seen = {alt.expression.strip() for alt in output if alt.expression.strip()}

    def add(expression: str, explanation: str) -> None:
        ...
        if not _is_valid_natural_alternative(
            ...
            return
        seen.add(expression)
        output.append(NaturalAlternative(expression=expression, explanation=explanation))

    time_match = re.search(
        r"((?:오늘|내일|이번 주|다음 주|월요일|화요일|수요일|목요일|금요일|토요일|일요일)?"
        r"(?:\s*(?:오전|오후))?\s*(?:한|두|세|네|다섯|여섯|일곱|여덟|아홉|열|열한|열두)\s*시(?:에)?)",
        reference_message,
    )
    time_phrase = re.sub(r"\s+", " ", time_match.group(1)).strip() if time_match else ""

    if "면담" in reference_message:
        if expected_norm == "formal":
            if time_phrase:
                add(
                    f"교수님, 가능하시다면 {time_phrase} 면담 일정을 조율하고 싶습니다.",
                    "직접적으로 허락을 요구하기보다 가능 여부를 전제로 말해 더 정중하게 들립니다.",
                )
                add(
                    f"교수님, {time_phrase} 면담이 가능하신지 여쭙고 싶습니다.",
                    "교수님께 시간을 문의할 때는 '여쭙고 싶습니다'가 더 공손한 표현입니다.",
                )
            add(
                "교수님, 가능하신 시간에 맞춰 면담 일정을 조율하고 싶습니다.",
                "상대의 일정을 우선하는 표현이라 공식적인 면담 요청에 잘 어울립니다.",
            )
        else:
            if time_phrase:
                add(
                    f"교수님, 혹시 {time_phrase} 면담 일정을 잡아도 괜찮을까요?",
                    "'혹시'와 '괜찮을까요'를 함께 쓰면 요청이 더 부드럽게 들립니다.",
                )
                add(
                    f"교수님, {time_phrase} 면담이 가능하신지 여쭤봐도 될까요?",
                    "가능 여부를 먼저 여쭤보는 표현이라 더 자연스럽고 공손합니다.",
                )
            add(
                "교수님, 가능하신 시간에 맞춰 면담 일정을 조율하고 싶어요.",
                "상대의 가능한 시간을 먼저 고려하는 표현이라 부담이 덜합니다.",
            )

    if "상담" in reference_message and ("방향성" in reference_message or "프로젝트" in reference_message):
        if expected_norm == "formal":
            add(
                "프로젝트 방향성에 대해 조언을 구하고 싶습니다.",
                "'상담하다' 대신 '조언을 구하다'를 쓰면 공식적인 요청 느낌이 더 살아납니다.",
            )
            add(
                "프로젝트 방향성에 관해 상담을 받고 싶습니다.",
                "'-에 관해'를 사용하면 상담 주제를 또렷하게 드러낼 수 있습니다.",
            )
        else:
            add(
                "프로젝트 방향성에 대해 조언을 구하고 싶어요.",
                "'상담하다' 대신 '조언을 구하다'를 쓰면 요청 의도가 더 분명해집니다.",
            )
            add(
                "프로젝트 방향성에 관해 상담을 받고 싶어요.",
                "'-에 관해'를 사용하면 상담 주제를 자연스럽게 밝힐 수 있습니다.",
            )

    if "보고서" in reference_message and "제출" in reference_message:
        if expected_norm == "formal":
            add(
                "이번 주 안에 보고서를 제출해야 하는데, 아직 자료가 충분하지 않습니다.",
                "'-해야 하는데'를 쓰면 의무와 어려운 상황을 자연스럽게 연결할 수 있습니다.",
            )
        else:
            add(
                "이번 주 안에 보고서를 제출해야 하는데, 아직 자료가 충분하지 않아요.",
                "'-해야 하는데'를 쓰면 의무와 어려운 상황을 부드럽게 연결할 수 있습니다.",
            )

    if "과제" in reference_message and ("제출" in reference_message or "내" in reference_message) and "늦" in reference_message:
        if expected_norm == "formal":
            add(
                "교수님, 과제를 늦게 제출해도 괜찮겠습니까?",
                "'돼요?'보다 '괜찮겠습니까?'를 쓰면 공식적인 허락 요청으로 더 정중하게 들립니다.",
            )
            add(
                "교수님, 과제 제출이 늦어져도 괜찮을지 여쭙고 싶습니다.",
                "'여쭙고 싶습니다'는 교수님께 가능 여부를 조심스럽게 묻는 표현입니다.",
            )
        else:
            add(
                "교수님, 과제를 늦게 제출해도 괜찮을까요?",
                "'괜찮을까요?'를 쓰면 허락을 부드럽게 묻는 느낌이 납니다.",
            )
            add(
                "교수님, 과제 제출이 조금 늦어져도 될지 여쭤봐도 될까요?",
                "'여쭤봐도 될까요?'는 상대의 입장을 먼저 고려하는 표현입니다.",
            )

    if "날씨" in reference_message and re.search(r"집(에|에서)\s*(있고|쉬고|머물고)\s*싶", reference_message):
        weather_time_match = re.search(r"(오늘|내일|요즘|이번 주)은", reference_message)
        weather_time = weather_time_match.group(1) if weather_time_match else "오늘"
        if expected_norm == "formal":
            add(
                f"{weather_time}은 날씨가 많이 추워서 집에서 쉬고 싶습니다.",
                "'집에 있다'보다 '집에서 쉬다'가 의도를 더 자연스럽게 전달합니다.",
            )
            add(
                f"{weather_time}은 많이 추워서 집에 머물고 싶습니다.",
                "반복되는 주제를 줄이고 핵심 의도를 간결하게 말할 수 있습니다.",
            )
        else:
            add(
                f"{weather_time}은 날씨가 많이 추워서 집에서 쉬고 싶어요.",
                "'집에 있다'보다 '집에서 쉬다'가 일상 대화에서 더 자연스럽습니다.",
            )
            add(
                f"{weather_time}은 많이 추워서 집에 머물고 싶어요.",
                "짧지만 의미가 분명한 표현이라 자연스럽게 들립니다.",
            )

    if len(output) < 2 and expected_norm == "formal":
        formalized_reference = replace_all(reference_message, [
            (r"돼요([?？])", r"되겠습니까\1"),
            (r"돼요", "됩니다"),
            (r"가능해요([?？]?)", r"가능하겠습니까\1"),
            (r"가능해요", "가능합니다"),
        ...
        add(
            formalized_reference,
            "합쇼체 상황에서는 문장 끝을 격식 있게 맞추면 더 자연스럽습니다.",
        )
    elif len(output) < 2 and expected_norm == "polite":
        add(
            reference_message.replace("되겠습니까", "될까요").replace("가능하십니까", "가능하실까요"),
            "해요체에서는 '-을까요?'를 쓰면 부드럽게 가능 여부를 물을 수 있습니다.",
        )
...
def prune_suspicious_corrections(
    ...
    for correction in corrections:
        ...
        if is_suspicious_inline_correction(original or corrected_message, corrected):
            continue
    ...
    for correction in candidates:
        ...
        for other in candidates:
            if other is correction:
                continue
            if corrected_message and other.corrected.strip() == corrected_message.strip():
                continue
            if len(other.original) <= len(correction.original):
                continue
            if other.type != correction.type:
                continue
...
def compose_corrected_message_from_corrections(
    ...
    for correction in applicable:
        if correction.original in rebuilt:
            updated = apply_inline_correction_once(rebuilt, correction.original, correction.corrected)
            if updated != rebuilt:
                rebuilt = updated
                changed = True
...
def is_covered_by_smaller_corrections(original: str, corrections: List[InlineCorrection]) -> bool:
    original_norm = re.sub(r"[.!?…。！？\s]+", "", original or "")
    if not original_norm:
        return False
    covered = [
        correction for correction in corrections
        if correction.original
        and correction.original != original
        and re.sub(r"[.!?…。！？\s]+", "", correction.original) in original_norm
    ]
    if len(covered) >= 2:
        return True
    covered_types = {correction.type for correction in covered}
    return CorrectionType.VOCABULARY in covered_types and CorrectionType.SPEECH_LEVEL in covered_types


def sync_whole_sentence_speech_corrections(
    corrections: List[InlineCorrection],
    original_message: str,
    corrected_message: Optional[str],
) -> List[InlineCorrection]:
    if not corrected_message:
        return corrections
    original_norm = (original_message or "").strip()
    corrected_norm = corrected_message.strip()
    if not original_norm or not corrected_norm:
        return corrections

    synced: List[InlineCorrection] = []
    for correction in corrections:
        if correction.type == CorrectionType.SPEECH_LEVEL and correction.original.strip() == original_norm:
            synced.append(InlineCorrection(
...
def choose_content_preserving_level_correction(
    user_message: str,
    expected_norm: str,
    local_rule_correction: RealTimeCorrection,
    corrections: List[InlineCorrection],
) -> str:
    """Pick a fallback that keeps the user's content instead of using canned examples."""
    candidates = [
        local_rule_correction.corrected_message,
        compose_corrected_message_from_corrections(user_message, corrections),
        make_level_suggestion(user_message, expected_norm),
    ...
    for candidate in candidates:
        ...
        if not candidate or candidate == original:
            continue
        if verify_with_rules(candidate, "") != expected_norm:
            continue
        if is_generic_greeting_correction(user_message, candidate):
            continue
        if is_suspicious_semantic_expansion(user_message, candidate):
            continue
        return candidate
    return original
...
def build_rule_based_correction(
    ...
    if corrected_message:
        for correction in corrections:
            if correction.type not in {
                CorrectionType.SPELLING,
                CorrectionType.GRAMMAR,
                CorrectionType.VOCABULARY,
                CorrectionType.HONORIFIC,
                CorrectionType.EXPRESSION,
            }:
                continue
            if correction.original in corrected_message:
                corrected_message = apply_inline_correction_once(
                    corrected_message,
                    correction.original,
                    correction.corrected,
                )
        corrections = sync_whole_sentence_speech_corrections(corrections, user_message, corrected_message)
...
"""
    ...
    def _apply_native_analyzer_feedback(
        ...
        for item in native.word_errors:
            ...
            if not original or not corrected:
                continue
            native_error_type = item.get("type")
            if native_error_type in {"directness", "wrong_level", "too_casual", "too_stiff", "mixed_level"}:
                correction_type = CorrectionType.SPEECH_LEVEL
            elif native_error_type == "honorific_missing":
                correction_type = CorrectionType.HONORIFIC
            else:
                correction_type = CorrectionType.VOCABULARY
            if correction_type == CorrectionType.SPEECH_LEVEL and expected_norm == "formal":
                formalized = make_level_suggestion(corrected, "formal")
                if formalized and formalized != corrected:
                    corrected = formalized
            if native_error_type == "directness" and is_covered_by_smaller_corrections(original, corrections):
                continue
            key = (original, corrected, correction_type)
    ...
    async def _analyze_realtime(
        ...
        result = await clova_service.analyze_json(prompt, temperature=0.2, max_tokens=1400)
        ...
        if not result:
            ...
            if local_rule_correction.corrected_message:
                local_rule_correction.corrections = merge_surface_corrections(
                    local_rule_correction.corrections,
                    user_message,
                    local_rule_correction.corrected_message,
                )
            local_rule_correction.natural_alternatives = build_rule_natural_alternatives(
                user_message,
                local_rule_correction.corrected_message or user_message,
                expected_norm,
                local_rule_correction.natural_alternatives,
            )
        ...
        for c in result.get("corrections", []) or []:
            try:
                ...
                if original not in user_message:
                    ...
                    continue
                if is_suspicious_inline_correction(original, corrected):
                    print(f"[ChatService] ⚠️ Semantic expansion skip: '{original}' -> '{corrected}'")
                    continue
                correction_type = CorrectionType(c.get("type", "grammar"))
                inferred_type = infer_surface_correction_type(original, corrected)
                if correction_type in {CorrectionType.SPELLING, CorrectionType.EXPRESSION} and inferred_type != CorrectionType.SPELLING:
                    correction_type = inferred_type
                elif correction_type == CorrectionType.SPELLING and is_particle_adjustment(original, corrected):
        ...
        if not is_level_correct:
            ...
            local_corrected_candidate = (local_rule_correction.corrected_message or "").strip()
            rule_corrected = (
                local_corrected_candidate
                if local_corrected_candidate and verify_with_rules(local_corrected_candidate, "") == expected_norm
                else simple_convert_to_level(user_message, expected_norm)
            )
            ...
            if preferred_rule:
                ...
                clova_corrected
                and clova_corrected_norm == expected_norm
                and not is_generic_greeting_correction(user_message, clova_corrected)
                and not is_suspicious_semantic_expansion(user_message, clova_corrected)
            ):
                ...
                if rule_corrected and verify_with_rules(rule_corrected, "") == expected_norm:
                    ...
                    best_corrected = choose_content_preserving_level_correction(
                        user_message=user_message,
                        expected_norm=expected_norm,
                        local_rule_correction=local_rule_correction,
                        corrections=corrections,
                    )
            ...
            if not has_whole_sentence_speech_correction:
                corrections.insert(0, InlineCorrection(
                    original=user_message,
                ...
                corrections = [
                    InlineCorrection(
                        ...
                        corrected=(
                            best_corrected
                            if c.type == CorrectionType.SPEECH_LEVEL and c.original.strip() == user_message.strip()
                            else c.corrected
                        ),
                        type=c.type,
                        severity=c.severity,
        ...
        corrected_message = choose_safe_corrected_message(
            user_message,
            (result.get("corrected_message") or "").strip(),
            local_rule_correction.corrected_message,
        )
        ...
        # Patch surface-level corrections into corrected_message even if the LLM
        # already set one. Guarded replacement avoids duplicates like 시간을을.
        if corrected_message:
            surface_corrections = [
                c for c in corrections
                if c.type in {
            ...
            for sc in surface_corrections:
                corrected_message = apply_inline_correction_once(corrected_message, sc.original, sc.corrected)
            corrections = sync_whole_sentence_speech_corrections(corrections, user_message, corrected_message)
            corrections = merge_surface_corrections(corrections, user_message, corrected_message)
            natural_alternatives = build_rule_natural_alternatives(
                user_message,
                corrected_message,
                expected_norm,
                natural_alternatives,
