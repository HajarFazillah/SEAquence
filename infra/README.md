...
아래 12개 테이블이 모두 보이면 정상입니다: `users`, `user_avatar`, `sessions`, `chat_turns`, `vocabulary`, `mistakes`, `password_reset_tokens`, `session_utterance`, `topic_preferences`, `relationship_analysis`, `session_mistakes`, `feedback`.
...
| `user_avatar` | 사용자 생성 아바타 (이름, 역할, 성격, 나이, 관계 설정 등) |
| `sessions` | 대화 세션 정보 (사용자, 아바타, 상황, 시작/종료 시간 등) |
| `chat_turns` | 세션별 대화 메시지 기록 (역할, 메시지, 턴 번호) |
| `vocabulary` | 사용자가 저장한 어휘/표현 (단어, 예문, 번역) |
| `mistakes` | 학습 중 발생한 교정 기록 (카테고리, 원문, 교정문) |
| `password_reset_tokens` | 비밀번호 재설정용 임시 인증 코드 |
| `session_utterance` | 세션별 발화(STT 전사) 기록 |
