...
// Extended Relationships with categories
...
interface AvatarFormData {
  ...
  role: string;
  customRole: string; // For free input
  relationship_description: string;
  description: string; // 아바타 관련 설명
...
export default function CreateAvatarScreen() {
  ...
  const initialData: AvatarFormData = existingAvatar || template || {
    name_ko: '',
    name_en: '',
    age: '',
    gender: 'other',
  ...
  const generateBio = () => {
    ...
    setTimeout(() => {
      ...
      const bio = `${formData.name_ko || '이 아바타'}는 ${roleLabel} 관계입니다. ` +
  ...
  const isStepValid = () => {
    switch (currentStep) {
      ...
      case 2:
        return formData.role.length > 0 || formData.customRole.trim().length > 0;
      case 3:
  ...
  // Get color palette chunks for display
  ...
  // Render Step 2: Relationship (Grid + Custom)
  const renderStep2 = () => (
    <View style={styles.stepContent}>
      ...
      <View style={styles.customRoleSection}>
        ...
        <View style={styles.customRoleRow}>
          <TextInput
            ...
            onChangeText={(v) => {
              updateField('customRole', v);
              if (v.trim()) updateField('role', '');
            }}
  ...
  // Render Step 3: Description (관계 설명 + 아바타 관련 설명)
  const renderStep3 = () => (
    <View style={styles.stepContent}>
      ...
      <TextInput
        ...
        placeholder="이 아바타와 어떤 관계인지 설명해주세요&#10;&#10;예: 같은 동아리에서 만난 후배, 회사에서 2년째 같이 일하는 동료"
        placeholderTextColor="#B0B0C5"
        multiline
        numberOfLines={3}
      ...
      <TextInput
        ...
        placeholder="이 아바타의 성격, 말투, 특징을 자세히 설명해주세요&#10;&#10;예: 이 사람은 항상 밝고 에너지가 넘칩니다. 말을 빠르게 하고 농담을 자주 해요. '진짜?', '대박!' 같은 감탄사를 자주 사용합니다."
        placeholderTextColor="#B0B0C5"
        multiline
        numberOfLines={5}
        textAlignVertical="top"
      ...
      <View style={styles.descHint}>
        ...
        <Text style={styles.descHintText}>
          이 설명을 바탕으로 AI가 아바타의 말투와 성격을 표현합니다
        </Text>
      </View>
    </View>
  );

  // Render Step 4: Personality & Interests
  const renderStep4 = () => (
    <View style={styles.stepContent}>
      ...
      <View style={styles.tagGrid}>
        {['정치', '종교', '논쟁', '스포츠', '연예인', '학업', '취업', '결혼', '외모', '돈'].map((item) => (
          <Tag
            key={item}
            label={item}
            selected={formData.dislikes.includes(item)}
            onPress={() => toggleArrayItem('dislikes', item)}
            variant="outline"
          />
        ))}
      </View>
          ...
          <View style={styles.previewTypeBadge}>
            {formData.avatarType === 'fictional' ? (
              <>
                ...
                <Text style={[styles.previewTypeText, { color: '#9C27B0' }]}>가상 인물</Text>
              </>
            ) : (
              <>
                <User size={12} color="#2196F3" />
                <Text style={[styles.previewTypeText, { color: '#2196F3' }]}>실제 인물</Text>
              </>
            )}
          </View>
        </View>
