...
export const useHomeData = () => {
  ...
  const [conversationPreviews, setConversationPreviews] =
    useState<Record<string, ConversationPreview>>({});
  ...
  const fetchAll = useCallback(async () => {
    try {
      setLoading(true);
      const [profileResult, statsResult, previewResult, vocabResult] =
  ...
  // Refetch when the home tab regains focus (e.g. returning from a session,
  // saving vocab in the post-chat summary, or deleting in saved-vocabulary).
  useFocusEffect(useCallback(() => { fetchAll(); }, [fetchAll]));

  return { profile, stats, conversationPreviews, loading, error };
};
